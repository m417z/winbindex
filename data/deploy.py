...
def build_html_index_of_hashes():
    ...
    file_names = sorted(info_sources)
    index_chars = string.ascii_lowercase + string.digits

    for char in index_chars + '_':
        html_code_start = (
            '<!DOCTYPE html>'
            '<html>'
            '<head>'
            '<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">'
            f'<title>{char.upper()} - Winbindex hashes</title>'
            '<style>'
            ...
            '</head>'
            '<body>'
            f'<h1>{char.upper()} - Winbindex hashes</h1>'
        )
        ...
        html_code_index = '<div>'
        for index_char in index_chars + '_':
            if index_char != char:
                html_code_index += f'<a href="{index_char}.html">{index_char.upper()}</a> '
            else:
                html_code_index += f'{index_char.upper()} '
        html_code_index += '</div><br>'

        if char != '_':
            iter_file_names = [name for name in file_names if name[0] == char]
        else:
            iter_file_names = [name for name in file_names if name[0] not in index_chars]

        html_code_main = '<div>'
        for name in iter_file_names:
            html_code_main += f'<h3><a href=".?file={html.escape(name)}">{html.escape(name)}</a></h3>'
            for file_hash in sorted(info_sources[name]):
                html_code_main += '<p>' + file_hash
            html_code_main += '</p>'
        html_code_main += '</div>'

        html_code = html_code_start + html_code_index + html_code_main + html_code_end
        with open(f'../hashes/{char}.html', 'w') as f:
            f.write(html_code)
